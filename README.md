...
ACT (Action Chunking with Transformers) 及其多任务变体 MTACT 在 LIBERO 仿真环境上的独立实现。
...
act-libero/
├── scripts/
│   ├── train_act.py          # ACT 训练 + 评估主入口
│   └── train_mtact.py        # MTACT 训练 + 评估主入口
├── configs/
│   ├── act_config.py         # ACT 超参数与路径配置
│   └── mtact_config.py       # MTACT 超参数（含 task embedding 相关字段）
├── models/
│   ├── act_policy.py         # ACTPolicy（纯 ACT，不含 task_emb）
│   ├── mtact_policy.py       # ACTPolicy（MTACT 版本，带 task_emb 参数）
│   └── detr/
│       ├── detr_vae.py       # DETRVAE CVAE 主体；暴露 build_act_model / build_mtact_model 两个工厂函数
│       ├── backbone.py       # ResNet 视觉骨干网络（基于 torchvision）
│       ├── transformer.py    # Transformer 编码器 / 解码器（task_emb 可选）
│       └── position_encoding.py  # 正弦 / 学习式位置编码
├── data/
│   ├── act_dataset.py        # ACT 数据集加载（返回 4-tuple）
│   └── mtact_dataset.py      # MTACT 数据集加载（返回 5-tuple，含 task_emb）
├── task_embeddings/          # 由 utils/gen_task_emb.py 生成的每个 task suite 的 task embedding .npy
├── utils/
│   ├── check_npz.py          # 查看 npz 缓存文件内容的调试工具
│   └── gen_task_emb.py       # 用 sentence-transformers 生成 task embeddings
...
将 `configs/act_config.py` 中的 `data_root` 设置为该目录的路径，`task_suite_name` 设置为对应子目录名即可。
...
所有参数统一在 `configs/act_config.py` 的 `ACT_LIBERO_CONFIG` 字典中配置。
...
1. 确认 `configs/act_config.py` 中的 `data_root` 指向正确的数据目录
...
```bash
python -m scripts.train_act --eval
```

### 指定特定 checkpoint

```bash
python -m scripts.train_act --eval --ckpt_name policy_best_rollout.ckpt
python -m scripts.train_act --eval --ckpt_name checkpoint_epoch_500.ckpt
python -m scripts.train_act --eval --ckpt_name policy_last.ckpt
```
...
MTACT 是 ACT 的多任务扩展，通过给模型额外注入 **task embedding** 让同一套参数能在多个任务上学习。

### 与 ACT 的差异

| 维度 | ACT | MTACT |
|---|---|---|
| 输入 | `qpos + image` | `qpos + image + task_emb (384-d)` |
| Policy 文件 | `models/act_policy.py` | `models/mtact_policy.py` |
| Dataset 返回 | 4-tuple | 5-tuple（多一个 `task_emb`） |
| Config | `configs/act_config.py` | `configs/mtact_config.py` |
| 训练脚本 | `scripts/train_act.py` | `scripts/train_mtact.py` |
| Transformer 输入前缀 | `[latent, proprio]` | `[latent, proprio, task_emb]` |
| 缓存文件 | 4 字段 | 5 字段（多一个 `task_name`） |

Task embedding 用 `sentence-transformers` 的 `all-MiniLM-L6-v2` 对每个 task 的自然语言描述编码，维度固定 384。`detr_vae.py` 内部通过 `multi_task` 开关区分两条代码路径，policy 层通过 `build_act_model` / `build_mtact_model` 两个工厂函数隔离。

### 生成 Task Embeddings

```bash
cd /local_data/jl17265/projects/act-libero
python -m utils.gen_task_emb --task_suite libero_spatial
python -m utils.gen_task_emb --task_suite libero_object
python -m utils.gen_task_emb --task_suite libero_goal
python -m utils.gen_task_emb --task_suite libero_10
```

输出默认落到 `./task_embeddings/{task_suite}_task_embeddings.npy`，是 `{task_description: np.ndarray(384,)}` 的字典。仓库已随源码附带 4 个 suite 的预生成 `.npy`，如需自定义 suite 或换模型再运行此脚本。

### MTACT 专属 Config 字段

`configs/mtact_config.py` 在 ACT 默认字段基础上新增：

| 字段 | 说明 | 默认值 |
|---|---|---|
| `multi_task` | 启用 task-embedding 分支（构建 `build_mtact_model`） | `True` |
| `task_emb_dim` | Task embedding 维度（必须与 `gen_task_emb.py` 一致） | `384` |
| `task_emb_path` | `{task_description: emb}` 字典的 `.npy` 路径 | `./task_embeddings/libero_spatial_task_embeddings.npy` |

### 训练

```bash
cd /local_data/jl17265/projects/act-libero
python -m scripts.train_mtact
```

### 评估

```bash
python -m scripts.train_mtact --eval
python -m scripts.train_mtact --eval --ckpt_name policy_best_rollout.ckpt
```

### 缓存不兼容

MTACT 的 `.npz` 缓存比 ACT 多一个 `task_name` 字段。如果复用了 ACT 遗留的缓存目录，MTACT 加载时会主动抛错并提示：

```
RuntimeError: Cache file episode_XXXX.npz is missing 'task_name'.
This cache was built by the ACT pipeline. Delete {cache_dir} and rerun to
regenerate an MTACT-compatible cache.
```

解决方法是删除 `{data_root}/{task_suite_name}_cache/` 后重新运行 MTACT 训练，让 `data/mtact_dataset.py` 重新从 TFRecord 生成。
...
由以下三部分拼接（定义于 `scripts/train_act.py`）：
...
在 `configs/act_config.py` 中通过 `temporal_agg` 切换：
