"""
DETR VAE model for ACT / MTACT policy.

The same DETRVAE class backs both ACT (single-task) and MTACT (multi-task with
sentence-transformer task embeddings). The ``multi_task`` flag controls whether
the extra ``text_embed_proj`` layer and the third ``additional_pos_embed`` slot
are instantiated. Policy code should pick the right factory:

    build_act_model(args)    -> multi_task=False
    build_mtact_model(args)  -> multi_task=True
"""
import torch
...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, action_dim, qpos_dim, num_queries, camera_names, multi_task=False, task_emb_dim=384):
        ...
        self.encoder_joint_proj = nn.Linear(qpos_dim, hidden_dim)
        ...
        if multi_task:
            ...
            self.text_embed_proj = nn.Linear(task_emb_dim, hidden_dim)
        else:
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, action_dim, qpos_dim, camera_names):
        super().__init__()
        self.camera_names = camera_names
...
def _build(args, multi_task):
    backbones = [build_backbone(args)]
    transformer = build_transformer(args)
    encoder = build_encoder(args)

    task_emb_dim = getattr(args, "task_emb_dim", 384)
    model = DETRVAE(
        backbones,
        transformer,
        encoder,
...
def build_act_model(args):
    return _build(args, multi_task=False)


def build_mtact_model(args):
    return _build(args, multi_task=True)
